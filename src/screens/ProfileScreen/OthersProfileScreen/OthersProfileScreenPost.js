...
const OthersProfileScreenPost = ({myPost,setMyPost,pageLoad,setPageLoad,totalPost,followingData,followersData,userProfile}) => {
  ...
  const LoadPost = async (page) => {
    ...
    const postDetails = { postType:0,role:userProfile?.user_role,circle_type:userProfile?.user_role == 4 ? 1 : userProfile?.circle_type,
      city_id:userProfile?.city_id,assoc_id:userProfile?.associationId,pageCounter:page,user_id:userProfile?.id,id:userProfile?.id
    }
    const result = await dispatch(getMyPostsApi(postDetails));
    setEndNull(result.payload.result)
    setPageLoad(result?.payload?.pageCounter)
  ...
  return (
    <View style={{backgroundColor:'#E6E6E6',flex:1}}>
        ...
        {/* <View style={{borderWidth:1,height:26,height:50,backgroundColor:"#000",marginTop:25}}>
...
export default OthersProfileScreenPost